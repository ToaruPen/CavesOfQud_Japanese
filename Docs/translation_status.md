...
| ObjectBlueprints | ??? | `ObjectBlueprints/Items.jp.xml` ? Batch6?18 ??????Batch18?`work/items_missing_batch18.json` ? `_translated_batch18.json`??? ????????????????glowcrust / fickle gill / mumble mouth / waxflab / Klanq?? Security Card / Trollking Key / BarathrumKey / CrystalKey / Grit Gate Grid Key ????????? Token?Clue?DataDisk ? 38 ??`py scripts/diff_localization.py --missing-only --base ObjectBlueprints/Items`?? 2025-11-12: Laser Rifle / Force Modulator / Bionic Hand 等の DisplayName を自動検出し、英語複合語に対応する位置へ中黒 `・` を挿入して命名規則を統一。2025-11-11: Love／Blaze／Hulk honey／Salve／Sphynx salt／Shade oil／Ubernostrum／Rubbergum／Nectar トニックの RulesDescription を日本語化し、クイックネス／筋力などの用語を整理。Steel Arrow を「鋼鉄の矢」に統一し、棍棒・モーター駆動キャタピラ・反応式トラウマプレートの説明も更新。|
