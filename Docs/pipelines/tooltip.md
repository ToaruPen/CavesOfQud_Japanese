...
> **Contract (2025.11)** TMP_Text.SetText/TMP_Text.text へ直接触れず、gameQueue の Look.GenerateTooltipContent と uiQueue の ModelShark.TooltipTrigger.SetText で翻訳を確定させる。Prefix で <color> などの RichText タグをサニタイズしてから TMP へ渡す。
...
- HookGuard: `TooltipParamMapCache` + `UIContext` ?? gameQueue ??? BodyText/Display ?????????`TooltipBodyTextCache` ?? RTF ?????`TMPTextSetTextPatch` ???g???X???[?g?O?A?h????? `%Field%` ???????? `TryRestorePlaceholders` ??????`TranslationContextGuards` ?? `ModelShark.Tooltip.*` ??????????d?q?[?X?L?b?v???B
## 現状メモ (2025-11-13)
- TooltipTrigger.SetText Prefix で TooltipFieldLocalizer を通し、DisplayName/LongDescription/Feeling 等を翻訳・サニタイズ済み文字列へ置換する。
- TooltipRenderGuardPatch では SetTextAndSize 後に TMP_Text を列挙し、FontManager.ApplyToText(..., forceReplace=true) で強制的に CJK フォントへ差し替える。DualPolatLooker などデュアル tooltip もここでフォントが適用される。
- 重量ラベル("重量：14ポンド") や武器カテゴリ行 (Strength ボーナス上限 等) は LongDescription に埋め込まれた原文を TooltipTextLocalizer で分割して翻訳している。まだ英語が残る箇所(重量単位、数値行の括弧)があるため、Label/Value を個別に正規化するルールを追加予定。
- ModelShark.Tooltip.* Context での RTF サニタイズ (空の <color> 等) が完了してから TMP へ渡すので、TMP 側の Hook は不要。
### Tooltip メモ (2025-11-13 更新)
- TooltipTrigger.SetText Prefix で TooltipFieldLocalizer を経由させ、DisplayName/LongDescription/Feeling/WoundLevel を翻訳＋正規化してから UI に渡している。
- TooltipRenderGuardPatch で SetTextAndSize 後の全 TMP_Text に FontManager.ApplyToText(..., forceReplace=true) を適用し、DualPolatLooker を含む Modern Tooltip でも必ず CJK フォントへ差し替えられる。
- 重量行やサブアーム攻撃率などの数値ラベルは LongDescription 側に埋まっているため、TooltipTextLocalizer のルールを追加して lbs. / Strength ボーナス などの表記を揺らさず処理する必要がある (現在 重量：14ポンド のような混在が残っている)。
- 今後は Label と Value を分離して正規化テーブルを整備し、英語の括弧や単位が残らないようにする。

### DisplayName タグの保持 (2025-11-13 追記)
- Armor / MeleeWeapon / MissileWeapon / Shield など多くの装備パーツは `GetDisplayNameEvent` で独自タグを付与する。例: Armor は `{{b|\u0004}}` (AV アイコン) と `{{K|\t}}` (DV アイコン) を挿入し、武器は命中・貫通・ダメージのアイコンやダイス表記を追加する。
- DisplayName を辞書で丸ごと置換するとこれらのタグが失われ、青いアイコンや `+1/0` などのステータス表示が消える。翻訳前にタグを抽出して保持し、翻訳後に名前へ再連結する仕組みを TooltipFieldLocalizer へ組み込むこと。
- Armor/MeleeWeapon 以外の装備 (MissileWeapon、銃、盾、サイバネ等) も `HandleEvent(GetDisplayNameEvent)` を洗い出し、タグ一覧を作成してルール化する。装備全般でタグを復元できるようにしないと表示欠落が再発する。
