...
| Manual | `references/Base/Manual.xml` | `Mods/QudJP/Localization/Manual.jp.xml` + Harmony patch (`Docs/manual/ManualPatch.jp.manual`) | ゲーム内ヘルプは `XmlDataHelper` 管理だが、実際の翻訳は Harmony (`ManualLocalizationPatch`). | Loader に渡す XML は空スタブでよい。本文は C# から注入。`Replace` 非対応。 | コメント: `Mods/QudJP/Localization/Manual.jp.xml:2-7` |
