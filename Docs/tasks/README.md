...
| --- | --- | --- |
| `Docs/tasks/ui.md` | Options / EmbarkModules など UI 系 XML | `python3 scripts/diff_localization.py --missing-only` で UI に関係するファイルが出たら追記 |
...
### 注意事項
- 自動レポート（`python3 scripts/diff_localization.py --missing-only --json-path Docs/backlog/latest.json` 等）がある場合は、その結果をもとに「未訳ファイル一覧」を上書きする。
